...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    if (command == "HAPTIC") {
      Serial.println("Received HAPTIC command from dashboard");
      Serial.println("Playing haptic feedback effect 47 (strong click)");
      playHapticSingle(47);  // Play once instead of loop
      Serial.println("HAPTIC_COMPLETE");
    } else if (command == "TEST") {
      Serial.println("Arduino is ready and listening");
    } else if (command.startsWith("HAPTIC_")) {
      // Allow specific effect numbers like "HAPTIC_42"
...
// -------- Haptic helper (single play) --------
void playHapticSingle(uint8_t fx) {
